...
class Gt2FCOSTarget(BaseOperator):
    ...
    def __call__(self, samples, context=None):
        assert len(self.object_sizes_of_interest) == len(
            self.downsample_ratios
...
@register_op
class Gt2TTFTarget_MULTI_CLS(Gt2TTFTarget):
    __shared__ = ["num_classes"]
    """
    Gt2TTFTarget
    Generate TTFNet targets by ground truth data
    ...
    def __init__(self, num_classes=80, down_ratio=4, alpha=0.54):
        super(Gt2TTFTarget_MULTI_CLS, self).__init__()
        self.down_ratio = down_ratio
        self.num_classes = num_classes
        self.alpha = alpha
